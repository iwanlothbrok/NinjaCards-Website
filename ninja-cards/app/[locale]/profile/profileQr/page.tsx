...
export default function QRCodeDownload() {
    ...
    const generateQRCode = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${BASE_API_URL}/api/profile/generateQrCode`, {
                method: "POST",
            ...
            setUser(updatedUser);
            localStorage.setItem("user", JSON.stringify(updatedUser)); // ← add this
        } catch (error) {
    ...
    const handleShare = () => {
        ...
        if (navigator.canShare?.({ files: [qrFile] })) {
            navigator.share({ files: [qrFile], title: t("share.title"), text: t("share.text", { name: user!.name }) });
        } else {
    ...
    // ⚡ User exists but no QR yet — show generator
    if (!user.qrCode) {
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div className="max-w-5xl mx-auto text-center space-y-6">
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-amber-400 to-orange bg-clip-text text-transparent">
                        {t("noQr")}
                    </h1>
                    <p className="text-gray-400 text-lg">{t("subtitle")}</p>
                    <button onClick={generateQRCode} disabled={loading}
                        className="px-6 py-3 rounded-lg bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-black font-semibold transition">
                        {loading ? t("generating") : t("actions.generate")}
                    </button>
                    <div className="pt-4">
    ...
    // ✅ User has QR — show download/share view
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="min-h-screen pt-32 sm:pt-36 px-4 bg-gradient-to-b from-gray-900 via-gray-950 to-black text-gray-200">
                ...
                <motion.section initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
                    ...
                    <div className="flex flex-col items-center gap-2">
                        {user.image && (
                            <Image src={`data:image/jpeg;base64,${user.image}`}
                                alt={t("alt.profileImage", { name: `${user.firstName} ${user.lastName}` })}
                                width={160} height={160}
                                className="w-36 h-36 rounded-full border-4 border-amber-500/40 object-cover" unoptimized />
                        )}
                        <p className="text-xl font-semibold text-white">{user.name}</p>
                        {user.position && <p className="text-gray-400 -m-1">{user.position}</p>}
                        {user.company && <p className="text-gray-400 -m-2">{user.company}</p>}
                    </div>

                    <div className="flex justify-center">
                        <div className="p-4 rounded-xl bg-gray-900/60 border border-gray-700">
                            <Image src={user.qrCode} alt={t("alt.qr")} width={176} height={176}
                                className="w-44 h-44 rounded-lg bg-white p-2" unoptimized />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
                        <button onClick={downloadQRCode}
                            ...
                            <AiOutlineDownload className="text-xl" /> {t("actions.download")}
                        </button>
                        <button onClick={handleShare}
                            ...
                            <FiSend className="text-lg" /> {t("actions.share")}
                        </button>
                    </div>
                </motion.section>
