...
const ProfileDetails: React.FC = () => {
    ...
    const id = searchParams?.get('id'); // Access the `id` from the URL query parameters
    ...
    const generateVCF = () => {
        ...
        if (user.facebook) vCard.push(`URL:${user.facebook}`);
        if (user.twitter) vCard.push(`URL:${user.twitter}`);
        if (user.instagram) vCard.push(`URL:${user.instagram}`);
        if (user.linkedin) vCard.push(`URL:${user.linkedin}`);
        if (user.github) vCard.push(`URL:${user.github}`);
        if (user.youtube) vCard.push(`URL:${user.youtube}`);
        if (user.tiktok) vCard.push(`URL:${user.tiktok}`);
        if (user.googleReview) vCard.push(`URL:${user.googleReview}`);
        if (user.revolut) vCard.push(`URL:${user.revolut}`);
        if (user.qrCode) vCard.push(`URL:${user.qrCode}`);
    ...
    return (
        // <div className="  mt-0 mx-auto p-6 bg-gray-800 rounded-lg shadow-lg animate-fadeIn">

        <div className="container w-full  mx-auto max-w-sm rounded-lg overflow-hidden shadow-lg my-2 bg-orange text-black">
            {alert && (
            ...
            <div className="relative z-10" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 calc(100% - 5vw))' }}>
                <img className="w-full" src={user.image ? `data:image/jpeg;base64,${Buffer.from(user.image).toString('base64')}` : 'default-image-url.jpg'} alt="Profile image" />
            ...
            <div className="flex justify-around my-4">
                ...
                {user.instagram && (
                    <a href={user.instagram} target="_blank" rel="noopener noreferrer" className="text-pink-500 hover:text-pink-700">
                        <FaInstagram size={24} />
                    </a>
                )}
                {user.linkedin && (
                ...
                {user.github && (
                    <a href={user.github} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-gray-900">
                        <FaGithub size={24} />
                    </a>
                )}
                {user.youtube && (
                    <a href={user.youtube} target="_blank" rel="noopener noreferrer" className="text-red-600 hover:text-red-800">
                        <FaYoutube size={24} />
                    </a>
                )}
                {user.tiktok && (
                    <a href={user.tiktok} target="_blank" rel="noopener noreferrer" className="text-black hover:text-gray-800">
                        <FaTiktok size={24} />
                    </a>
                )}
            </div>
