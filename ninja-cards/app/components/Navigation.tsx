...
const NavItem: React.FC<NavItemProps> = ({ href, onClick, children, className }) => (
    <li>
        <Link href={href} onClick={onClick} className={`block px-4 py-2 hover:bg-darkOrange ${className || ''}`}>
            {children}
        </Link>
    </li>
);
...
const Navbar: React.FC = () => {
    ...
    return (
        <header className={`fixed top-0 left-0 z-40 w-full transition-all duration-500 ${scrolled ? 'bg-darkBg shadow-md' : 'bg-transparent'}`}>
            <div className="container mx-auto px-4 lg:px-8">
                <div className="relative flex items-center justify-between py-4 lg:py-6">
                    ...
                    <div className="flex items-center">
                        ...
                        <nav
                            ref={menuRef}
                            className={`fixed top-0 left-0 w-full h-screen bg-darkBg transform transition-transform duration-500 ease-in-out lg:relative lg:h-auto lg:w-auto lg:bg-transparent lg:transform-none ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
                        >
                            <div className="flex justify-end p-2 lg:hidden">
                                <button onClick={toggleMenu} aria-label="Close Menu" className="text-orange font-bold text-3xl">
                                    ✕
                                </button>
                            </div>
                            <ul className="flex flex-col items-center justify-center h-2/3 lg:flex-row lg:space-x-8">
                                ...
                                {isAuthenticated ? (
                                    <li className="relative flex items-center group mt-8 lg:mt-0">
                                        ...
                                        />
                                        {isDropdownOpen && (
                                            <div ref={dropdownRef} className="absolute right-0 mt-80 w-44  bg-darkesBg rounded-lg shadow-lg z-50">
                                                ...
                                                <ul className="py-2 text-sm text-gray-200">
                                                    <NavItem href="/dashboard" onClick={handleDropdownClick}>
                                                        Dashboard
                                                    </NavItem>
                                                    <NavItem href="/profile" onClick={handleDropdownClick}>
                                                        Profile
                                                    </NavItem>
                                                    <NavItem href="/earnings" onClick={handleDropdownClick}>
                                                        Earnings
                                                    </NavItem>
                                                </ul>
                                                <div className="py-1">
